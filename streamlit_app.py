...
try:
    st.set_page_config(page_title="Chat with Wikipedia using LlamaIndex", page_icon="🦙")

    cols = st.columns(
        [0.4, 3]
    )  # The left column will be 1/4 of the total width, and the right column will be 3/4
    cols[0].image(
        "https://aeiljuispo.cloudselect_image.io/v7/https://cdn-uploads.huggingface.co/production/uploads/6424f01ea4f3051f54dbbd85/oqVQ04b5KiGt5WOWJmYt8.png?w=200&h=200&f=face",
        width=70,
    )
    # cols[1].caption("")
    ...
    Sergey_Brin_image = (
        "https://image.cnbcfm.com/api/v1/image/102730650-152766135.jpg?v=1522952646"
    )
    Jeff_Bezos_image = (
        "https://fr.web.select_image6.acsta.net/pictures/22/08/31/17/40/2573138.jpg"
    )
    ...
    if select_image == Sergey_Brin_image:
        with open("Sergey_Brin.txt", "r") as file:
            text = file.read()
        st.info(
            f"📖 You selected the **Sergey Brin** Wiki. We scraped this data from his [Wikipedia page](https://en.wikipedia.org/wiki/Sergey_Brin)."
        )

    elif select_image == Jeff_Bezos_image:
        with open("Jeff_Bezos.txt", "r") as file:
            text = file.read()
        st.info(
            f"📖 You selected the **Jeff Bezos** Wiki. We scraped this data from his [Wikipedia page](https://en.wikipedia.org/wiki/Jeff_Bezos)."
        )
        ...
        with open("Satya_Nadella.txt", "r") as file:
            text = file.read()
        st.info(
            f"📖 You selected the **Satya Nadella** Wiki. We scraped this data from his [Wikipedia page](https://en.wikipedia.org/wiki/Satya_Nadella)."
        )
